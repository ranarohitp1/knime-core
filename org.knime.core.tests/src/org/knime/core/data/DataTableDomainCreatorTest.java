...
public class DataTableDomainCreatorTest {
    ...
    public void testMetaData() {
        ...
        DataTableDomainCreator domainCreator = new DataTableDomainCreator(tableSpec, false);
        assertThat(getMetaData(domainCreator), is(new TestMetaData(Collections.emptyList())));

        domainCreator.updateDomain(new DefaultRow("test", "A"));
        assertThat(getMetaData(domainCreator), is(new TestMetaData(Arrays.asList("A"))));

        domainCreator.updateDomain(new DefaultRow("test", "B"));
        assertThat(getMetaData(domainCreator), is(new TestMetaData(Arrays.asList("A", "B"))));
    }

    private static TestMetaData getMetaData(final DataTableDomainCreator domainCreator) {
        DataColumnSpec spec = domainCreator.createSpec().getColumnSpec(0);
        Optional<TestMetaData> optMetaData = spec.getMetaDataOfType(TestMetaData.class);
    ...
    public void testInitMetaData() {
        ...
        TestMetaData metaData = new TestMetaData(values);
        ...
        concat.add("A");
        domainCreator.updateDomain(new DefaultRow("test", "A"));
        assertThat(getMetaData(domainCreator), is(new TestMetaData(concat)));
    }
